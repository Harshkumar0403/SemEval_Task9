...
INPUT_FILE = "Dataset/Subset/urd_200.tsv"
PRED_DIR = "llama/train/zeroshot"
PRED_FILE = f"{PRED_DIR}/pred_urd.csv"
EVAL_FILE = f"{PRED_DIR}/eval_urd.txt"
...
# ---------------------------------------------------------
SYSTEM_PROMPT = """
You are an expert analyst of Urdu and Roman Urdu social media discourse for SemEval 2026 
Task 9 (Subtask 2): Polarization Type Classification.

Detect ONLY explicit hostility expressed toward identifiable groups.

VALID LABELS:
Political, Racial/Ethnic, Religious, Gender/Sexual, Other, None

DEFINITIONS:
• Political: hostility toward political parties, leaders, voters, ideology, institutions
  (e.g., patwari, youthia, establishment).
• Racial/Ethnic: hostility toward ethnic groups or national identities (Punjabi, Sindhi,
  Mohajir, Pashtun, Afghan, Bengali).
• Religious: hostility toward religious groups, sects, clergy, beliefs, or institutions.
• Gender/Sexual: misogyny, harassment, homophobia, gender-based insults.
• Other: hostility toward any other identifiable group.
• If no explicit group-directed hostility exists, output: None.

IMPORTANT:
• Urdu posts often contain sarcasm, rhetorical attacks, humor, and emojis—these count only if
  directed at a group.
• Mentioning a group does NOT automatically imply polarization.
• Multi-label outputs are common and should be included whenever justified.
• Emojis alone do not imply hostility.

OUTPUT FORMAT (strict JSON):
{"labels": "Political, Religious, Racial/Ethnic"}
{"labels": "None"}
"""
...
# ---------------------------------------------------------
def main():
    print("\n=== Zero-Shot LLAMA Classification (Urdu) ===\n")

    df = pd.read_csv(INPUT_FILE, sep="\t")
    preds, gold = [], []

    total = len(df)
    print(f"Processing {total} Urdu samples...\n")
