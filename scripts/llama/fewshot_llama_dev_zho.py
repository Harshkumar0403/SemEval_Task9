...
# ============================================================
#       CHINESE SYSTEM PROMPT (FEWSHOT VERSION)
# ============================================================
...
Chinese posts in this dataset are often extremely short (sometimes only 1–3 characters),
but may still contain strong group-based hostility. Many hostile expressions appear as 
abbreviations, slang, or implicit references.

You must classify ONLY explicit hostility directed at identifiable groups.

VALID LABELS:
Political, Racial/Ethnic, Religious, Gender/Sexual, Other, None

DEFINITIONS:
• Political — attacks on political parties, governments, officials, ideologies.
• Racial/Ethnic — hostility toward ethnic groups, nationalities, migrants, etc.
• Religious — hostility toward religious groups, beliefs, institutions.
• Gender/Sexual — misogyny, harassment, LGBTQ+ hostility.
• Other — hostility toward any identifiable social group not covered above.
• None — no explicit hostility.

RULES:
• Do NOT guess implied meaning; classify only explicit hostility.
• Short Chinese posts may contain indirect group markers—label only when clear.
• Emojis or numbers alone do NOT indicate hostility.
...
Now classify the following Chinese text. Return ONLY this JSON:
...
def main():
    print("\n=== FEW-SHOT LLAMA DEV INFERENCE (CHINESE / ZHO) ===")

    df = pd.read_csv(DEV_FILE)

    # remove empty label columns
