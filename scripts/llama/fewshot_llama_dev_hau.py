...
DEV_FILE = "Dataset/dev_phase/subtask2/dev/hau.csv"
TRAIN_FILE = "Dataset/Processed/hau.tsv"

OUT_DIR = "llama/dev/fewshot"
os.makedirs(OUT_DIR, exist_ok=True)

OUTPUT_FILE = f"{OUT_DIR}/hau.csv"
...
# ============================================================
#       SYSTEM PROMPT (HAUSA — MATCH TRAINING STYLE)
# ============================================================

SYSTEM_PROMPT = f"""
You are an expert analyst of Hausa social media discourse. You classify Hausa posts for 
SemEval 2026 Task 9 (Subtask 2): Polarization Type Classification.

Hausa social media often includes:
• political insults or party-based attacks (APC, PDP, government, leaders),
• ethnic hostility, regional stereotyping,
• religious conflict and sectarian criticism (Muslims, Christians, Izala, Shi'a),
• gender-based insults or harassment,
• code-switching between Hausa and English,
• strong emotional tone, sarcasm, and exaggeration.

Your task is to detect ONLY explicit hostility directed at identifiable groups.

VALID LABELS:
Political, Racial/Ethnic, Religious, Gender/Sexual, Other, None

DEFINITIONS:
• Political — hostility toward political parties, leaders, governments, movements.
• Racial/Ethnic — hostility toward Hausa, Fulani, Yoruba, Igbo, Nigerien, etc.
• Religious — hostility toward Muslims, Christians, clerics, sects.
• Gender/Sexual — misogyny, harassment, anti-LGBTQ language.
• Other — any group-based hostility not fitting above.
• None — if no explicit group-targeted hostility exists.

IMPORTANT:
• Do NOT infer hidden meaning; classify only explicit hostility.
• Hausa discourse often uses jokes, exaggeration, and sarcasm — classify it only if a group 
  is clearly targeted.
...
Classify the next text. Return ONLY this JSON format:
...
def main():
    print("\n=== FEW-SHOT LLAMA DEV INFERENCE (HAUSA / HAU) ===")

    df = pd.read_csv(DEV_FILE)

    # Remove placeholder gold columns if present
