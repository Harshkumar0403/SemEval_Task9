...
INPUT_FILE = "Dataset/Subset/fas_200.tsv"
PRED_DIR = "llama/train/zeroshot"
PRED_FILE = f"{PRED_DIR}/pred_fas.csv"
EVAL_FILE = f"{PRED_DIR}/eval_fas.txt"
...
# ---------------------------------------------------------
SYSTEM_PROMPT = """
You are an expert analyst of Persian (Farsi) social media discourse and classify short posts
...
DEFINITIONS:
• Political: hostility involving government, political leaders, institutions, protests, ideology.
• Racial/Ethnic: insult or hostility between ethnic groups or nationalities.
• Religious: hostility involving religious identity, clerics, beliefs, or practices.
• Gender/Sexual: misogyny, gender insults, homophobia, etc.
• Other: explicit group hostility not covered above.
• If no explicit hostility is present, output: None.

IMPORTANT:
• Persian posts often use sarcasm or humor; do NOT label polarization unless hostility is clear.
• Mentioning religion, ethnicity, or politics alone does NOT imply polarization.
• Multi-label classification is allowed only when justified.
• Emojis (😂🤦💔) modify tone but do NOT indicate a label.
...
# ---------------------------------------------------------
def main():
    print("\n=== Zero-Shot LLAMA Classification (Persian) ===\n")

    df = pd.read_csv(INPUT_FILE, sep="\t")
    preds, gold = [], []

    total = len(df)
    print(f"Processing {total} Persian samples...\n")
