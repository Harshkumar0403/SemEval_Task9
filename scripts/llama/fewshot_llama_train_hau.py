...
SUBSET_FILE = "Dataset/Subset/hau_200.tsv"

OUTPUT_DIR = "llama/fewshot"
PRED_FILE = f"{OUTPUT_DIR}/pred_hau.csv"
EVAL_FILE = f"{OUTPUT_DIR}/eval_hau.txt"
...
# =========================================================
# LOAD TRAIN DATA & CREATE FEW-SHOT SAMPLES
# =========================================================
...
# Hausa is highly imbalanced → few-shot must be balanced artificially
sample_plan = {
    "Political": 2,
    "Racial/Ethnic": 2,
    "Religious": 2,
...
for label, count in sample_plan.items():
    # If bucket doesn't have enough examples, take all.
...
# =========================================================
SYSTEM_PROMPT = f"""
You are an expert analyst of Hausa social-media discourse.
Your task is SemEval 2026 Task 9 (Subtask 2): Polarization Type Classification.

Hausa posts often include humor, teasing, emojis, sarcasm, and informal speech. 
Most Hausa posts do NOT contain polarization. Only classify explicit hostility 
...
DEFINITIONS:
• Political: hostility toward gwamnati, jam’iyyu, shugabanni, ideology.
• Racial/Ethnic: hostility toward kabilu, ethnic groups, nationalities.
• Religious: hostility toward addini, sects, clergy.
• Gender/Sexual: misogyny or gender harassment.
• Other: any other group-level hostility.
• None: no explicit hostility (common for Hausa posts).

IMPORTANT RULES:
• Jokes, friendly insults, laughter emojis (😂🤣) do NOT indicate hostility.
• Do NOT over-classify — None is valid and common.
• Multi-label allowed ONLY if multiple groups are explicitly targeted.
• Only explicit hostility counts.
...
# =========================================================
def main():
    print("\n=== FEW-SHOT LLAMA Classification (Hausa) ===\n")
