...
INPUT_FILE = "Dataset/Subset/spa_200.tsv"
PRED_DIR = "llama/train/zeroshot"
PRED_FILE = f"{PRED_DIR}/pred_spa.csv"
EVAL_FILE = f"{PRED_DIR}/eval_spa.txt"
...
# ---------------------------------------------------------
SYSTEM_PROMPT = """
You are an expert analyst of Spanish social media discourse for SemEval 2026 Task 9 
...
DEFINITIONS:
• Political: hostility toward political parties, government, ideology, politicians, protests, or
  public institutions.
• Racial/Ethnic: insults toward immigrants, nationalities, minorities, or regional identities.
• Religious: hostility toward religious institutions, beliefs, clergy, or groups.
• Gender/Sexual: misogyny, homophobia, anti-feminism, LGBTQ+ hostility, gender-based insults.
• Other: explicit hostility toward any other social group.
• If no explicit group-directed hostility exists, output: None.

IMPORTANT:
• Spanish posts often contain sarcasm, humor, and insults; label polarization ONLY when directed at a group.
• Mentioning a group does NOT automatically imply polarization.
• Multi-label is allowed only when clearly justified.
• Emojis do not indicate hostility by themselves.
...
# ---------------------------------------------------------
def main():
    print("\n=== Zero-Shot LLAMA Classification (Spanish) ===\n")

    df = pd.read_csv(INPUT_FILE, sep="\t")
    preds, gold = [], []

    total = len(df)
    print(f"Processing {total} Spanish samples...\n")
